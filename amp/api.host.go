...
// Host allows app and transport services to be attached.  Child processes
// attach as it responds to client requests to "pin" nodes via URLs.
type Host interface {
	task.Context
	...
	// StartNewSession creates a new host session, binding the specified Transport
	// to it.
...
type TransportInfo struct {
	...
	// If set, the host must perform challenge-response authentication before
	// granting access.  True for remote transports (e.g. TCP), false for
	// local/embedded transports (e.g. lib).
	RequiresAuth bool
...
// Transport wraps a TxMsg transport abstraction, allowing a Host to connect
// over any data transport layer.  For example, tcp_service and lib_service each
// implement amp.Transport.
type Transport interface {

	// Returns the parameters describing this transport's capabilities and
	// requirements.
	Info() TransportInfo

	// Called when this Transport should close because the associated parent host
	// session is closing or has closed.
	Close() error
...
type HostService interface {
	...
	// This service may retain the amp.Host instance so that it can make calls to
	// StartNewSession().
	StartService(on Host) error

	// StopService initiates a polite stop of this extension and blocks until it's
	// in a "soft" closed state,
	//    meaning that its service has effectively stopped but its Context is still open.
	// Note this could take any amount of time (e.g. until all open requests are
	// closed) Typically, StopService() is called (blocking) and then
	// Context.Close().  To stop immediately, Context.Close() is always available.
	StopService()
}
...
type TxReceiver interface {
	...
	// Queues the given tx appropriately, aborting if ctx.Done() is signaled and
	// returns ctx.Err()
...
type Session interface {
	...
	// Submits a tx to this Session for processing, including who will receive
	...
	// Returns the current PlanetEpoch for a joined planet, or nil if not
	// registered.
	...
	// planet UID) watches it so its planet-public records stream in.  Distinct
	// from HostSession.SetPlanet, which joins with an epoch + keys.  No-op without
	// a vault transport.
	...
	// StoreBlob hashes and stores blob data locally, returning a populated
	// BlobRef.  The blob is stored encrypted in the host's BlobStore and queued
	// for peer propagation when the BlobRef is later committed in a TxMsg via
	// SubmitTx.
	//
	// meta describes the blob's MIME type (ContentType), human label (Text), and
	// byte size (I with Units = Bytes, used as the progress denominator); or may
	// be nil.  The stored BlobRef's AssetTag carries ContentType and Text from
	// meta; AssetTag.UID is the leading 16 bytes of the plaintext hash
	// (content-addressed, AOM SD-security-sync.md §13.2), and for a planet-public
	// blob BlobTag.UID coincides with it.  BlobTag itself stays lean — UID +
	// stored byte count.
	//
	// For large files, data is streamed — not buffered in memory.  If onProgress
	// is non-nil, it is called periodically with cumulative bytes written.
	StoreBlob(planetID tag.UID, src io.Reader, meta *Tag, onProgress func(bytesWritten int64)) (*BlobRef, error)

	// SeedBlob introduces a local file into a planet's blob pipeline. The host
	// opens the file directly (no IPC memcpy), hashes-and-stores in a single
	// streaming pass, and returns a populated BlobRef. Caller is expected to
	// upsert the BlobRef into whatever attr is appropriate on a target node (e.g.
	// std.Attr.NodeBlobs keyed by BlobTag.UID).
	//
	// Content-addressed: re-seeding the same file produces the same BlobRef and is
	// a no-op at the BlobStore layer (AOM SD-security-sync.md §13.2). ContentType
	// is inferred from the file extension.
	SeedBlob(planetID tag.UID, path string) (*BlobRef, error)

	// BlobStore returns the session's BlobStore for retrieving blobs by (planetID,
	// blobID).  Apps use this to build data.Asset instances backed by stored
	// blobs.
	BlobStore() BlobStore

	// OpenBlob resolves a BlobRef to a seekable plaintext reader — the read-side
	// twin of StoreBlob.  An epoch-sealed blob is retrieved as ciphertext,
	// decrypted once under its epoch key, and the recovered plaintext is validated
	// against the asset hash (Hash_0..3, AOM SD-security-sync.md §13.5); a public
	// blob streams straight from the BlobStore. Decrypted plaintext is served from
	// a Tier-2 cache so repeat reads (e.g. HTTP range requests while scrubbing
	// media) skip the decrypt. Apps use this to back a data.Asset over a stored
	// blob.
	OpenBlob(planetID tag.UID, ref *BlobRef) (data.AssetReader, error)

	// PrefetchBlobs names refs as admitted for the planet and pulls any still
	// missing — the admission surface for a playback-queue or spatial-neighborhood
	// prefetch signal (AOM SD-planet-storage.md §13.5, D17).  Best-effort and
	// asynchronous; a no-op without vault sync.
...
// per-concern downcast interfaces.  The host's session is the sole
// implementation.
type HostSession interface {

	// key custody

	// Returns the session's Enclave (identity key store), or nil if not yet
	// initialized.
	Enclave() safe.Enclave

	// Sets the session's Enclave. Called by the home app after opening/creating
	// it.
	SetEnclave(enc safe.Enclave)

	// Returns the session's EpochKeyStore (symmetric epoch keys), or nil if not
	// yet initialized.
	EpochKeys() safe.EpochKeyStore

	// Sets the session's EpochKeyStore. Called by the home app after
	// opening/creating it.
	SetEpochKeys(eks safe.EpochKeyStore)

	// planet / epoch control

	// Registers or updates a planet's epoch in this session.  First call for a
	// given planetID also joins the planet on the vault controller.
	//
	...
	// Processes a verified planet-public governance TxMsg (e.g. MemberEpoch
	// distribution).  Called by the vault controller after signature verification
	// succeeds.  Routes the TxMsg to all registered governance handlers for epoch
	// key extraction.
...
// KeyAdmission is the login boundary's ruling on member-signing-key custody for
// one session.  At most one of AdoptDeclared / MintNodeKey is acted on by
...
// structurally, never by flag convention (AOM SD-security-sync.md §8.5).
// NodeOperator is NOT key custody: it carries the transport's operator-rail
// declaration (TransportInfo.OperatorRail) through the session, and MintNodeKey
// never implies it — a challenge-free login is a custody fact, not operator
// trust.
...
// ACCEngine is the host's access-control resolver: it answers "who may do what"
// from a planet's verified governance state (channel epochs + the immutable
// founder set).  A Session exposes it OFF the public amp.Session interface for
// first-party governance apps (members, home) to consult.  The host's ACC
// engine is the sole implementation.  See AOM SD-channel-governance.md.
type ACCEngine interface {

	// ChannelEpoch returns the latest governing ChannelEpoch for (planetID,
	// nodeID), or nil when the channel is ungoverned.
	ChannelEpoch(planetID, nodeID tag.UID) *ChannelEpoch

	// HasAccess reports whether memberID holds at least `required` access on the
	// channel.
	HasAccess(planetID, nodeID, memberID tag.UID, required Access) bool

	// ResolveAccess returns memberID's effective Access on the channel
...
// range queries.  This is the primary storage for the vault sync engine — it
// preserves the original wire-format TxMsg bytes for signature verification and
// peer-to-peer propagation.
//
// Quarantine: entries that fail cryptographic verification (bad MemberProof or
// bad signature) can be marked quarantined with a TTL.  Quarantined entries are
// suppressed from ReadSince (and therefore from fanout + sync propagation)
// while remaining retrievable via ReadQuarantined for audit.  HighWater and
// RangeHash continue to include quarantined entries so peers converge on the
// same journal shape.  The underlying store evicts quarantined entries after
// TTL.
type TxJournal interface {
	...
	// Quarantine rewrites the existing entry for (planetID, txTimeID) with a
	// quarantine flag and the given TTL.  Returns ErrNotFound-style status if the
	// entry is absent.
	Quarantine(planetID tag.UID, txTimeID tag.UID, ttl time.Duration) error

	// ReadQuarantined iterates quarantined entries strictly after the `after`
	// mark.  Parallel to ReadSince; returning false from the callback stops
	// iteration.
	ReadQuarantined(planetID tag.UID, after tag.UID, cb func(txTimeID tag.UID, raw []byte) bool) error
}

// TxOutbox queues locally authored TxMsgs for propagation to vaults.  Entries
// persist across restarts — the outbox is drained when vault connectivity is
// available.  Blob bytes never queue: the relayed TxMsg is the announce, and
// receivers pull what its BlobRef ops name (receiver-driven transfer, AOM
...
// BlobStore manages content-addressed encrypted blob storage.  Blobs are
// identified by (PlanetID, BlobID) and validated by their content hash.
type BlobStore interface {

	// Store writes data under a caller-supplied blobID without deriving or
	// validating it — the raw primitive beneath StoreHashed (derives the ID from
	// the plaintext hash) and StoreValidated (validates the stream against an
	// existing BlobTag.UID before publish).  Callers that already hold the content
	// address use it directly; most callers want StoreHashed or StoreValidated.
	...
	// StoreHashed hashes and stores planet-public data content-addressed in a
	// single streaming pass.  Caller pre-populates ref.PlanetID_0/1, ref.HashKitID
	// (0 = the default kit), and optionally the asset identity
	// ref.AssetTag.ContentType() / ref.AssetTag.Text.  On success, StoreHashed
	// populates ref.Hash_0..3 from the content hash and both content addresses
	// (AOM SD-security-sync.md §13.2): the asset identity AssetTag (UID = leading
	// 16 bytes of the plaintext hash, I / Units = Bytes = authoritative plaintext
	// byte count) and the lean storage identity BlobTag.  A public blob's stored
	// bytes are the plaintext, so BlobTag.UID == AssetTag.UID.  Idempotent: if the
	// hash-derived on-disk path already exists, the temp write is discarded.
	StoreHashed(ref *BlobRef, data io.Reader, onProgress func(bytesWritten int64)) error

	// StoreValidated streams a peer-supplied blob into a temp file under the
	// planet dir, hashing it with ref.HashKitID, and publishes it at the
	// content-addressed path only when the streamed bytes satisfy
	// hash(stream)[:16] == ref.BlobTag.UID() — the address of the bytes as stored
	// (ciphertext for a sealed blob, plaintext for a public one), validated
	// without the epoch key (AOM SD-security-sync.md §13.2.1). This is the
	// receiver's O(1)-memory ingest: no whole-blob buffer, atomic temp+rename
	// mirroring StoreHashed. On hash mismatch or I/O error the temp is discarded
	// and an error returned, so the durable store and any presence tracking are
	// never poisoned by a partial or invalid transfer. It validates against the
	// existing BlobTag.UID and does not recompute the asset identity
	// (ref.Hash_0..3 / AssetTag — the member's concern, resolved from the sealed
	// TxMsg). Idempotent: an existing final path discards the temp and returns
	// nil.
...
type Registry interface {
	...
	// Registers a value as a prototype with a UID This allows the value to be
	// instantiated and unmarshaled when an AttrID is known.
...
// Request is a client request to pin a node or URL, offering many degrees of
// flexibility.
type Request struct {
