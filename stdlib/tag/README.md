...
- **`tag.Parse(str)`** — for ANY string that came from outside the program (wire, file, CLI flag, user input). Auto-detects format: a 26-char Base32 string decodes back to its original UID; anything else is canonicalized. **Inbound UIDs round-trip intact** — they're not re-hashed.
