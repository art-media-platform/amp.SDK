...
- **`tag.Parse(str)`** — for ANY string that came from outside the program (wire, file, CLI flag, user input). Auto-detects format: a Base32 UID string (grouped or solid) decodes back to its original UID; anything else is canonized. **Inbound UIDs round-trip intact** — they're not re-hashed.
