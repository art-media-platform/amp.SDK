...
/*****************************************************
** PayloadPacker signs and packs payload buffers IAW SigHeader
**/
...
func NewPacker(threadsafe bool) PayloadPacker {
...
func (P *PayloadPacker) ResetSession(
	...
	*P.signingKeyRef = KeyRef{}
	...
	keyEntry, err := enclave.FetchKeyInfo(signingKey)
	if err != nil {
		return err
	}
	...
	P.hashKit, err = NewHashKit(hashKit)
	if err != nil {
		return err
	}
	...
	P.signingKeyRef.KeyringID_0 = signingKey.KeyringID_0
	P.signingKeyRef.KeyringID_1 = signingKey.KeyringID_1
	P.signingKeyRef.PubKey = keyEntry.PubKey
	proto.Merge(P.signingKey, keyEntry)

	if outKeyInfo != nil {
		proto.Reset(outKeyInfo)
		proto.Merge(outKeyInfo, keyEntry)
	}
...
// PackAndSign signs a hash digest and packages it along with the payload and encoding into a single composite buffer
...
func (P *PayloadPacker) PackAndSign(
	...
	bufSz := 60 + int(hdr.HeaderSz) + int(hdr.BodySz) + 100 + extra
	buf := make([]byte, bufSz)

	// 1) Marshal the SigHeader and write its length
	sigHdrSz := 0
	buf, err = data.MarshalTo(buf[2:], &hdr)
	if err != nil {
		return status.Code_MarshalFailed.Errorf("failed to marshal SigHeader")
	}
	...
	if len(out.Hash) != P.hashKit.HashSz {
		return status.Code_AssertFailed.Error("hasher returned bad digest length")
	}

	// 5) Sign the hash
	signOut, err := P.enclave.DoCryptOp(&CryptOpArgs{
		Op:    CryptOp_Sign,
		Input: out.Hash,
	...
	sigLen := uint64(len(signOut.Output))
	if sigLen > 255 {
		return status.Code_AssertFailed.Error("unexpected sig length")
	}
	buf[pos] = byte(sigLen)
	pos++

	// 6) Append the sig
	copy(buf[pos:], signOut.Output)
...
func (P *PayloadPacker) checkReady() error {
	...
	if P.hashKit.Hasher == nil {
		return status.Code_NotReady.Error("payload hasher not set")
	}

	if P.enclave == nil {
		return status.Code_NotReady.Error("SKI signing session missing")
	}

	if P.signingKeyRef.KeyringID().IsNil() {
		return status.Code_NotReady.Error("signer keyring name missing")
	}

	if P.signingKey.CryptoKitID == 0 {
		return status.Code_NotReady.Error("signing key CryptoKit not set")
	}
...
func (U *PayloadUnpacker) UnpackAndVerify(
	...
	if bufLen < 50 {
		err = status.Code_UnmarshalFailed.Errorf("signed buf is too small (len=%v)", bufLen)
	}
	...
	if err == nil {
		...
		if sigHdrEnd > bufLen-10 {
			err = status.Code_UnmarshalFailed.Error("payload pos exceeds buf size")
		}
	}

	if err == nil {
		err = proto.Unmarshal(inSignedBuf[2:sigHdrEnd], &hdr)
		if err != nil {
			err = status.Code_UnmarshalFailed.Error("failed to unmarshal ski.SigHeader")
		}
	}
	...
	if err == nil {
		if bodyEnd > bufLen-1 {
			err = status.Code_UnmarshalFailed.Error("body end exceeds buf size")
		}
	}
