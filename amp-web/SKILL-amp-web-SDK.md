...
| **Verb-RPC** | A `POST /api/v1/tx` carrying `InvokeURL: amp://~/{app}/{verb}` — routes the batch to an app's verb handler (host-internal `TxCommit.Invoke`, not journaled) which authors the durable write custodially; the write path for `Access_ReadOnly` channels. SDK `invoke()` (§4.3). |
